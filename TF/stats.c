...
// Prefixo IPv4 da rede do túnel (conforme enunciado)
#define TUNNEL_PREFIX_V4 "172.31.66."

// Prefixo IPv6 (Geralmente Link-Local 'fe80:' em túneis simples ou 'fd00:'/'2001:' se configurado)
// Ajuste aqui se o seu IPv6 for diferente.
...
// // Prefixo da rede do túnel conforme enunciado (172.31.66.xxx)
// #define TUNNEL_PREFIX "172.31.66."
...
// // --- Funções Auxiliares (Listas Ligadas) ---

// // Função para encontrar ou criar um novo cliente
// ClientStats* get_or_create_client(const char *ip) {
//     ClientStats *current = client_list_head;
...
//     ClientStats *new_client = (ClientStats*)malloc(sizeof(ClientStats));
//     if (new_client == NULL) {
//         perror("Erro ao alocar memoria para ClientStats");
//         return NULL;
//     }
...
// // Função para encontrar ou criar estatísticas remotas para um cliente
// RemoteStats* get_or_create_remote_stats(ClientStats *client, const PacketInfo *info) {
//     RemoteStats *current = client->remote_head;
    
//     // 1. Tenta encontrar estatística remota existente
//     while (current != NULL) {
//         if (strcmp(current->remote_ip, info->ip_dst) == 0 &&
//             current->remote_port == info->port_dst &&
//             strcmp(current->protocol, info->trans_protocol) == 0) {
...
//     // 2. Cria nova estatística remota
//     RemoteStats *new_remote = (RemoteStats*)malloc(sizeof(RemoteStats));
//     if (new_remote == NULL) {
//         perror("Erro ao alocar memoria para RemoteStats");
//         return NULL;
//     }
...
//     // Adiciona no início da lista remota do cliente
//     new_remote->next = client->remote_head;
//     client->remote_head = new_remote;
...
// void init_stats() {
//     // Limpa a tela e mostra status
//     printf("\033[2J"); 
//     printf("\033[H");  
//     printf("Monitor de Tráfego Inicializado. Filtrando rede: %s*\n", TUNNEL_PREFIX);
//     fflush(stdout);
// }
...
//     // Atualiza contadores globais (opcional: se quiser contar tudo, mova para antes do filtro)
//     total_packets_received++;
//     total_bytes_received += info->total_len;
    
//     // --- FILTRAGEM ---
//     // Só processa se a ORIGEM for um IP da rede 172.31.66.x
//     // Isso evita criar entradas para servidores externos (Google, etc.)
//     if (strncmp(info->ip_src, TUNNEL_PREFIX, strlen(TUNNEL_PREFIX)) != 0) {
//         return;
//     }

//     // 1. Encontra/Cria o cliente (Source IP)
//     ClientStats *client = get_or_create_client(info->ip_src);
    
//     if (client) {
//         // 2. Atualiza contadores específicos do cliente
//         client->total_packets++;
//         client->total_bytes += info->total_len;

//         // 3. Atualiza estatísticas do destino remoto (apenas protocolos de transporte relevantes)
//         if (strcmp(info->trans_protocol, "TCP") == 0 || 
//             strcmp(info->trans_protocol, "UDP") == 0 ||
//             strcmp(info->trans_protocol, "SCTP") == 0) 
//         {
...
// /**
//  * Desenha a interface modo texto usando códigos de escape ANSI.
//  */
// void draw_interface() {
//     // Códigos ANSI: Limpar tela e mover cursor para (1, 1)
//     printf("\033[2J"); 
//     printf("\033[H"); 
    
//     // Título
//     printf("\033[1;32m=== Monitor de Tráfego (Filtro: %s*) ===\033[0m\n", TUNNEL_PREFIX);
    
//     // Estatísticas Globais
//     printf("Total Pacotes: \033[1m%lu\033[0m | Total Bytes: \033[1m%lu\033[0m\n\n", 
//            total_packets_received, total_bytes_received);
           
//     // Tabela de Clientes
//     printf("------------------------------------------------------------------------\n");
//     printf("| %-18s | %-15s | %-12s | %-15s |\n", 
//            "CLIENTE (TUNEL)", "REMOTO", "PROTOCOLO", "PACOTES/BYTES");
//     printf("------------------------------------------------------------------------\n");

//     ClientStats *client = client_list_head;
    
//     if (client == NULL) {
//         printf("| Aguardando tráfego de clientes na rede %s...                |\n", TUNNEL_PREFIX);
//     }

//     while (client != NULL) {
//         // Linha do Cliente
//         printf("| \033[1;36m%-18s\033[0m | TOTAL: %-19s | %-15lu |\n", 
//                client->client_ip, " ", client->total_bytes);
...
//         while(remote != NULL) {
//             // Limita a 5 destinos por cliente para não poluir demais a tela
//             if (remote_count < 5) {
//                 printf("| %-18s | %s:%-8d | %-12s | %lu/%lu |\n",
//                     " ", remote->remote_ip, remote->remote_port, remote->protocol,
//                     remote->packet_count, remote->byte_count);
//             }
//             remote = remote->next;
//             remote_count++;
...
// void cleanup_stats() {
//     ClientStats *current_client = client_list_head;
//     ClientStats *next_client;
    
//     while (current_client != NULL) {
//         next_client = current_client->next;
        
//         RemoteStats *current_remote = current_client->remote_head;
//         RemoteStats *next_remote;
//         while (current_remote != NULL) {
//             next_remote = current_remote->next;
//             free(current_remote);
//             current_remote = next_remote;
//         }
...
//         current_client = next_client;
//     }
//     printf("\nMemória de estatísticas liberada.\n");
// }
...
// // void init_stats() {
// //     // Não faz nada aqui, pois ncurses não pode ser usado.
// //     // Apenas imprime um cabeçalho inicial (ANSI)
// //     printf("\033[2J"); // Limpa a tela
// //     printf("\033[H");  // Move o cursor para o topo
// //     printf("Monitor de Tráfego Inicializado. Aguardando pacotes...\n");
// //     fflush(stdout);
// // }
...
// //     if (client) {
// //         // 2. Atualiza contadores globais do cliente
// //         client->total_packets++;
// //         client->total_bytes += info->total_len;

// //         // 3. Atualiza estatísticas do destino remoto (se for TCP/UDP/SCTP)
// //         if (strcmp(info->trans_protocol, "TCP") == 0 || 
// //             strcmp(info->trans_protocol, "UDP") == 0 ||
// //             strcmp(info->trans_protocol, "SCTP") == 0) 
// //         {
...
// //     // Título
// //     printf("\033[1;32m=== Monitor de Tráfego de Rede (ANSI) ===\033[0m\n");
...
// //     if (client == NULL) {
// //         printf("| Nenhuma atividade de cliente registrada.                                |\n");
// //     }
...
// //         // Destinos Remotos (Requisito do volume de tráfego associado)
// //         RemoteStats *remote = client->remote_head;
// //         while(remote != NULL) {
// //             printf("| %-18s | %s:%-8d | %-12s | %lu/%lu |\n",
// //                    " ", remote->remote_ip, remote->remote_port, remote->protocol,
...
// //     // Garante que todos os dados sejam escritos no terminal imediatamente
// //     fflush(stdout); 
// // }
...
// //     // Libera a memória de todas as estruturas
// //     while (current_client != NULL) {
// //         next_client = current_client->next;
        
// //         // Libera a lista de destinos remotos
// //         RemoteStats *current_remote = current_client->remote_head;
// //         RemoteStats *next_remote;
// //         while (current_remote != NULL) {
// //             next_remote = current_remote->next;
